...
USAGE:
    Basic usage with a single file:
        python normalize_rms_folder.py path/to/audio.wav --target-rms 0.1

    Normalize all audio in a folder:
        python normalize_rms_folder.py path/to/folder --target-rms 0.1

    Specify custom output directory:
        python normalize_rms_folder.py path/to/folder --target-rms 0.1 --output-dir path/to/output

ARGUMENTS:
    path                    Input audio file or folder to process
    --target-rms           Target RMS level in linear full-scale (required)
                           Examples:
                             0.1   = -20 dBFS
                             0.05  = -26 dBFS
                             0.01  = -40 dBFS
    --output-dir          Optional output directory. Defaults to a sibling folder
                          named "normalized_rms" or "{folder_name}_normalized_rms"

EXAMPLES:
    Normalize a single file to -20 dBFS:
        python normalize_rms_folder.py "C:\\audio\\speech.wav" --target-rms 0.1

    Normalize all files in a folder to -26 dBFS:
        python normalize_rms_folder.py "C:\\audio\\stimuli" --target-rms 0.05

    Normalize and save to custom folder:
        python normalize_rms_folder.py "stimuli" --target-rms 0.1 --output-dir "normalized"

SUPPORTED FORMATS:
    .wav, .flac, .ogg, .aiff, .aif, .au
"""
