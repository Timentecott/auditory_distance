...
def resolve_supported_output_sample_rate(device, requested_rate, channels=4, dtype='float32', label='output'):
    """Return the first supported sample rate for a device, trying requested/default/common fallbacks."""
    candidates = [int(requested_rate)]
    try:
    ...
    for fallback_rate in (48000, 44100):
        if fallback_rate not in candidates:
            candidates.append(fallback_rate)

    last_error = None
    for rate in candidates:
        try:
            sd.check_output_settings(device=device, channels=channels, dtype=dtype, samplerate=rate)
            if rate != requested_rate:
                print(f"Info: {label} sample rate changed from {requested_rate} Hz to {rate} Hz for device {device}.")
            return rate
        except Exception as exc:
            last_error = exc

    raise RuntimeError(
        f"No supported output sample rate found for device {device}. "
...
def run_loudness_calibration(win, headphones_device, speakers_device, sample_rate=48000):
    ...
    intro_speaker_file = Path(r"C:\Users\tim_e\source\repos\auditory_distance\experiment_1\loudspeaker_stimuli_2906\noise\brown_noise_5s.wav")
    headphone_file = Path(r"C:\Users\tim_e\source\repos\auditory_distance\experiment_1\ex_situ_stimuli_2906\noise\brown_noise_5s.wav")
    speaker_file = Path(r"C:\Users\tim_e\source\repos\auditory_distance\experiment_1\loudspeaker_stimuli_2906\noise\brown_noise_5s.wav")
    ...
    if headphones_device != 20 or speakers_device != 20:
        print("Warning: ASIO channel routing is configured for device index 20.")

    sample_rate = resolve_supported_output_sample_rate(
        speakers_device,
        sample_rate,
        channels=4,
        dtype='float32',
...
in_situ_headphone_dir = os.path.join(base_dir, 'in_situ_stimuli_2906')
ex_situ_headphone_dir = os.path.join(base_dir, 'ex_situ_stimuli_2906')
speaker_dir = os.path.join(base_dir, 'loudspeaker_stimuli_2906')
...
sample_rate = resolve_supported_output_sample_rate(
    ASIO_AGGREGATE_DEVICE,
    sample_rate,
    channels=4,
    dtype='float32',
    label='experiment playback'
)
...
try:
    for p in range(practice_trials):
        ...
        try:
            ...
            if fs != sample_rate:
                n_samples = int(round(audio_5s.shape[0] * (sample_rate / fs)))
                if audio_5s.ndim == 1:
                    audio_5s = signal.resample(audio_5s, n_samples)
                else:
                    audio_5s = np.column_stack([
                        signal.resample(audio_5s[:, ch], n_samples) for ch in range(audio_5s.shape[1])
                    ])
                fs = sample_rate
