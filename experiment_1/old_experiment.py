...
def apply_fade(audio, sample_rate=44100, fade_ms=10):
...
def append_silence_tail(audio, sample_rate=44100, tail_ms=20):
...
def play_audio_on_device(audio, sample_rate=44100, device_index=None, mapping=None):
...
def apply_bandpass_filter(audio, sample_rate=44100, hp_freq=80, lp_freq=10000, order=4):
    """
...
def run_loudness_calibration(win, headphones_device, speakers_device, sample_rate=44100):
...
def create_playback_streams(headphones_device, speakers_device, sample_rate=44100):
...
headphone_dir = os.path.join(base_dir, 'in_situ_stimuli_252')
speaker_dir = os.path.join(base_dir, 'loudspeaker_stimuli_252')
...
def _design_peaking_sos(f0_hz, gain_db, q, sample_rate=44100):
...
def apply_individual_eq(audio, eq_file, sample_rate=44100):
...
for p in range(practice_trials):
    ...
    try:
        ...
        sd.play(routed_audio, samplerate=44100, device=device)
        start_time = time.time()
        audio_duration = len(routed_audio) / 44100
