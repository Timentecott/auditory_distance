...
class stimulus:
    ...
    def generate(self, fs, duration, amplitude, repetitions, silenceAtStart, silenceAtEnd,sweeprange):
        ...
        if self.type == 'sinesweep':
            ...
            sinsweep = np.tile(sinsweep, (repetitions, 1))
        ...
        if self.type == 'sinesweep':
            ...
            for idx in range(0,numChans):
                ...
                if self.repetitions > 1:
                    currentChannel = currentChannel[:self.repetitions*self.Lp]
                    sig_reshaped = currentChannel.reshape((self.repetitions,self.Lp))
                    rir_repetitions = np.zeros((self.repetitions, tmplen))

                    for rep_idx in range(self.repetitions):
                        rir_repetitions[rep_idx,:] = fftconvolve(self.invfilter, sig_reshaped[rep_idx,:])

                    RIRs[:,idx] = np.mean(rir_repetitions, axis=0)
                else:
